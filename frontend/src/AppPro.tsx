...
export default function AppPro() {
  ...
  const loadAll = useCallback(async () => {
    try {
      setError(null);
      const [status, overviewData, timelineData, narrativeData, networkData, demoData, alertData, eventData, collectorData] = await Promise.all([
      ...
      setConnectors(status.connectors || []); setOverview(overviewData); setTimeline(timelineData.points || []); setNarratives(narrativeData.narratives || []); setNetwork(networkData); setDemographics(demoData); setAlerts(alertData.alerts || []); setEvents(eventData.events || []); setCollector(collectorData);
      setSelectedEventId((current) => eventData.events.some((item) => item.id === current) ? current : eventData.events[0]?.id || null);
      const inferred = String(eventData.events[0]?.public_profile?.search_query || '').trim(); if (inferred) setActiveQuery(inferred);
      const preferred = selectedNarrativeId || narrativeData.narratives[0]?.id || null;
      if (preferred) { setSelectedNarrativeId(preferred); try { setNarrative(await api.narrative(preferred)); } catch { setNarrative(null); } } else setNarrative(null);
    } catch (err) { setError(err instanceof Error ? err.message : 'Unable to reach NEXUS backend.'); }
  }, [selectedNarrativeId]);

  useEffect(() => { void loadAll(); }, []);
  useEffect(() => { if (!collector?.running) return; const timer = window.setInterval(() => void loadAll(), 15000); return () => window.clearInterval(timer); }, [collector?.running, loadAll]);
  ...
  const action = async (label: string, fn: () => Promise<unknown>) => { setLoading(true); setError(null); setNotice(null); try { await fn(); setNotice(label); await loadAll(); } catch (err) { setError(err instanceof Error ? err.message : 'Action failed.'); } finally { setLoading(false); } };
  const freshSearch = async () => { const clean = query.trim(); if (!clean) return; setLoading(true); setError(null); setNotice(null); try { const result = await api.searchWorkspace(clean, { reset: true, limitPerSource: 15 }); const ok = Object.entries(result.sources).filter(([, status]) => status.state === 'OK').map(([name]) => name); const failed = Object.entries(result.sources).filter(([, status]) => status.state !== 'OK').map(([name]) => name); setActiveQuery(clean); setSelectedNarrativeId(null); setNarrative(null); setSelectedEventId(null); setNotice(`Fresh search: ${result.inserted} post(s) · OK ${ok.join(', ') || 'none'}${failed.length ? ` · unavailable ${failed.join(', ')}` : ''}`); await loadAll(); setTab('posts'); } catch (err) { setError(err instanceof Error ? err.message : 'Fresh search failed.'); } finally { setLoading(false); } };
  const openNarrative = async (id: string) => { setLoading(true); setError(null); try { setSelectedNarrativeId(id); setNarrative(await api.narrative(id)); setNetwork(await api.network(id)); setTab('narrative'); } catch (err) { setError(err instanceof Error ? err.message : 'Could not load narrative.'); } finally { setLoading(false); } };
  const openPost = (id: string) => { setSelectedEventId(id); setTab('posts'); };
  const importJson = async (event: ChangeEvent<HTMLInputElement>) => { const file = event.target.files?.[0]; event.target.value = ''; if (!file) return; setLoading(true); try { const parsed = JSON.parse(await file.text()); const rows = Array.isArray(parsed) ? parsed : parsed.events; if (!Array.isArray(rows)) throw new Error('JSON must contain an event array.'); await api.importEvents(rows); setNotice(`Imported ${rows.length} record(s) as IMPORT/REPLAY.`); await loadAll(); } catch (err) { setError(err instanceof Error ? err.message : 'Import failed.'); } finally { setLoading(false); } };
